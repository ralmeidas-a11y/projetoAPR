...
interface TechnicalExecutionPanelProps {
  ...
  onBack: () => void;
  onStatusUpdate: (id: string, status: StudyStatus) => void;
  onUpdateData?: (updatedData: FormData) => void;
}

export const TechnicalExecutionPanel: React.FC<TechnicalExecutionPanelProps> = ({ data, allRequests = [], onBack, onStatusUpdate, onUpdateData }) => {
  const [activeTab, setActiveTab] = useState(0); // 0: Dados do Estudo, 1: Análise Técnica, 2: Resposta
  const [activeFolder, setActiveFolder] = useState('Solicitação');
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && onUpdateData) {
      ...
      const newFiles = rawFiles.map((f: File) => ({
        name: f.name,
        size: f.size,
        type: f.type,
      ...
      // Persist attachments directly into the corresponding SharePoint subfolder
      (async () => {
        try {
          const shareRoot = await (window as any).api?.getSharePointPath?.();
          if (!shareRoot) return;
          const targetDir = `${shareRoot}/solicitantes/${data.requesterName}/${data.studyNumber}/${activeFolder}`;

          for (const f of rawFiles) {
            try {
              // If native path available (Electron), copy from disk
              // @ts-ignore
              if ((f as any).path) {
                // @ts-ignore
                await (window as any).api.saveFile((f as any).path, `${targetDir}/${f.name}`);
                console.log(`✓ Arquivo do analista copiado: ${f.name}`);
              } else {
                // Read as base64 and send
                const toBase64 = (fileObj: File) => new Promise<string>((resolve, reject) => {
                ...
                await (window as any).api.saveFileData(f.name, base64, targetDir);
                console.log(`✓ Arquivo do analista enviado por base64: ${f.name}`);
              }
            } catch (innerErr) {
              console.warn('Erro ao persistir arquivo do analista:', innerErr);
            }
          }
        } catch (err) {
          console.warn('Erro ao acessar SharePoint para persistir anexos:', err);
        }
      })();
    }
  };
  ...
  const handleViewFile = async (file: any) => {
    try {
      // Primeiro tentar ler do SharePoint caminho esperado
      const shareRoot = await (window as any).api?.getSharePointPath?.();
      if (!shareRoot) {
        alert('Caminho do SharePoint não disponível');
        return;
      }
      const targetPath = `${shareRoot}/solicitantes/${data.requesterName}/${data.studyNumber}/${activeFolder}/${file.name}`;
      const result = await (window as any).api.readFile(targetPath);
      if (result && result.success && result.base64) {
        setFilePreview({ name: result.name || file.name, base64: result.base64, mime: result.mime || 'application/octet-stream' });
        return;
      }

      // Se falhar, e o arquivo estiver apenas em memória (ex: carregado agora), tentar ler via FileReader se tivermos referência
      alert('Arquivo não encontrado no SharePoint. Talvez ainda não tenha sido salvo.');
    } catch (err) {
  ...
  const handleFinishStudy = () => {
    if (onUpdateData) onUpdateData({ ...data, totalExecutionTime: elapsedTime });
    onStatusUpdate(data.id, StudyStatus.CONTROLE_QUALIDADE);
  ...
  const revisionHistory = useMemo(() => {
    ...
    const baseCode = data.studyNumber.split('-REV')[0].replace('PROV-', '');
  ...
  const renderTechnicalField = (label: string, value: any, unit: string = '') => (
    <div className="flex flex-col p-4 bg-white rounded-2xl border border-slate-100 shadow-sm transition-all hover:shadow-md">
      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{label}</span>
      <span className="text-sm font-black text-[#004080]">{value} <span className="text-[10px] font-normal text-slate-400">{unit}</span></span>
    </div>
  );
  ...
  const getFilesForActiveFolder = () => {
    if (activeFolder === 'Solicitação') return data.selectedFiles || [];
    return data.categorizedFiles?.[activeFolder] || [];
  };

  const renderTabContent = () => {
    switch (activeTab) {
      case 0: // Dados do Estudo
        return (
          <div className="space-y-6 animate-in fade-in duration-300 overflow-y-auto h-full pr-2 pb-12 custom-scrollbar">
            {/* Revisão / Histórico Alert */}
            {(data.studyType === 'Revisão de Estudo' || revisionHistory.length > 0) && (
              <div className="bg-orange-50 border border-orange-200 rounded-[2rem] p-6 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-5">
                   <div className="w-14 h-14 bg-orange-100 text-orange-600 rounded-[1.25rem] flex items-center justify-center border border-orange-200 shadow-inner">
                      <i className="fa-solid fa-clock-rotate-left text-2xl"></i>
                   </div>
                   <div>
                      <h4 className="text-xs font-black text-orange-800 uppercase tracking-widest">Controle de Revisões</h4>
                      <p className="text-[11px] text-orange-700/80 font-bold uppercase mt-1">Este processo possui <span className="underline decoration-2">{revisionHistory.length} versões</span> de histórico associadas.</p>
                   </div>
                </div>
                <button 
                  onClick={() => setShowHistoryModal(true)}
                  className="px-8 py-3 bg-white border border-orange-200 text-orange-600 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-orange-600 hover:text-white transition-all shadow-sm active:scale-95"
                >
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm">
              <h4 className="text-[11px] font-black text-[#004080] uppercase tracking-widest mb-6 flex items-center gap-3">
                ...
                Identificação e Solicitante
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-400 font-bold uppercase mb-1.5">Solicitante</span>
                  <span className="text-sm font-black text-slate-700">{data.requesterName || '-'}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-400 font-bold uppercase mb-1.5">Área / Regional</span>
                  <span className="text-sm font-black text-slate-700">{data.requesterArea || '-'}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-400 font-bold uppercase mb-1.5">Contato Direto</span>
                  <span className="text-sm font-black text-slate-700 truncate">{data.email || '-'}</span>
                </div>
              </div>
            </div>

            {/* Localização e Comentários Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm">
                ...
                <div className="space-y-6">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-slate-400 font-bold uppercase mb-1.5">Nome/Razão Social</span>
                    <span className="text-sm font-black text-[#004080] leading-tight">{data.studyTitle || data.clientName || data.uteName || '-'}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-8">
                    <div className="flex flex-col">
                      <span className="text-[10px] text-slate-400 font-bold uppercase mb-1.5">Município</span>
                      <span className="text-sm font-black text-slate-700">{data.city || '-'}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] text-slate-400 font-bold uppercase mb-1.5">Bairro</span>
                      <span className="text-sm font-black text-slate-700">{data.neighborhood || '-'}</span>
                    </div>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] text-slate-400 font-bold uppercase mb-1.5">Endereço de Fornecimento</span>
                    <span className="text-xs font-bold text-slate-500 leading-relaxed border-l-2 border-slate-100 pl-4">{data.address || '-'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm flex flex-col">
                ...
                <div className="flex-grow p-6 bg-slate-50/60 rounded-[2rem] text-[11px] font-medium text-slate-600 italic leading-relaxed border border-slate-100 shadow-inner">
                  {data.comments || "O solicitante não forneceu notas técnicas adicionais para esta solicitação."}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm">
              <h4 className="text-[11px] font-black text-[#004080] uppercase tracking-widest mb-6 flex items-center gap-3">
                ...
                Dados Técnicos e Fluxos de Carga
              </h4>
              {renderCargaTable()}
            </div>
          </div>
        );
      case 1: // Análise Técnica
        return (
          <div className="h-full flex flex-col animate-in fade-in duration-300">
             <div className="bg-white rounded-[2.5rem] border border-slate-200 p-12 flex-grow shadow-sm flex flex-col items-center justify-center text-center">
                ...
                <h4 className="text-2xl font-black text-[#004080] uppercase tracking-tight mb-4">Workspace de Engenharia</h4>
                <p className="text-sm text-slate-500 max-w-lg mb-12 leading-relaxed font-medium">
                   Utilize as ferramentas de rede oficiais para realizar os cálculos. Todos os arquivos de simulação e memórias devem ser salvos na pasta <span className="font-bold text-[#004080] underline">Cálculos</span> à esquerda.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-lg">
                   <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl group hover:border-[#004080] transition-all cursor-pointer">
                      <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest group-hover:text-[#004080] block mb-2">Simulador Desktop</span>
                      <p className="text-xs font-black text-[#004080] uppercase">Synergi Gas 14.1</p>
                   </div>
                   <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl group hover:border-[#004080] transition-all cursor-pointer">
                      <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest group-hover:text-[#004080] block mb-2">Plataforma Web</span>
                      <p className="text-xs font-black text-[#004080] uppercase">GeoGas Connect 3D</p>
                   </div>
                </div>
             </div>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-slate-50 font-sans animate-in fade-in duration-300 overflow-hidden">
                  ...
                  revisionHistory.map((h) => (
                    <div key={h.id} className="p-8 bg-slate-50 border border-slate-100 rounded-[2rem] flex items-center justify-between group hover:bg-white hover:border-indigo-100 hover:shadow-xl transition-all border-l-8 border-l-indigo-500">
                       <div className="flex items-center gap-8">
                          ...
                          <div>
                             ...
                             <div className="flex gap-4 mt-1.5">
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1.5"><i className="fa-solid fa-calendar text-[8px]"></i> {formatDateBR(h.requestDate)}</p>
                       ...
                       <div className="flex items-center gap-6">
                          ...
                          <button onClick={() => setPreviewStudy(h)} className="px-6 py-3 bg-white border border-slate-200 text-[#004080] rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-all shadow-sm">
                             Visualizar
                          </button>
                       </div>
                    </div>
                 ...
                 <div className="grid grid-cols-2 gap-10">
                    <div className="space-y-6">
                       ...
                       <div className="grid grid-cols-2 gap-4">
                          ...
                          {renderTechnicalField('Data', formatDateBR(previewStudy.requestDate))}
      ...
      <div className="bg-white px-8 py-6 flex items-center border-b border-slate-200 shadow-sm z-10">
        ...
        <div className="flex items-center gap-3">
           <button onClick={onBack} className="w-12 h-12 rounded-2xl bg-slate-50 text-slate-400 hover:text-[#004080] border border-slate-100 flex items-center justify-center transition-all shadow-sm" title="Minimizar">
             <i className="fa-solid fa-window-minimize -mt-1.5"></i>
           </button>
      ...
      <div className="flex-grow flex flex-col md:flex-row overflow-hidden bg-slate-50/20">
        ...
        <div className="w-full md:w-64 bg-white border-r border-slate-200 p-6 flex flex-col gap-6">
          <div className="space-y-2">
            <h5 className="px-3 text-[9px] font-black text-slate-300 uppercase tracking-widest mb-4 flex items-center gap-2">
              <i className="fa-solid fa-layer-group text-[8px]"></i> Fluxo
            </h5>
            {[
              { label: 'Dados do Estudo', icon: 'fa-file-invoice' },
              { label: 'Análise Técnica', icon: 'fa-chart-network' },
              { label: 'Resposta', icon: 'fa-file-signature' }
            ].map((tab, idx) => (
              <button 
                key={tab.label} 
          ...
          <div className="flex-grow flex flex-col min-h-0">
             ...
             <div className="space-y-1 overflow-y-auto pr-1 custom-scrollbar">
               {['Solicitação', 'Resposta', 'Cálculos', 'Outros'].map(t => (
                  <button 
                    key={t} 
                    onClick={() => setActiveFolder(t)}
                    className={`w-full p-3 rounded-xl flex items-center justify-between text-[11px] font-black uppercase tracking-widest transition-all ${activeFolder === t ? 'text-orange-600 bg-orange-50/50' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'}`}
                  >
                    <div className="flex items-center gap-3">
                    ...
                    {activeFolder === t && (
                      <div onClick={(e) => { e.stopPropagation(); handleAttachFile(); }} className="w-6 h-6 bg-white rounded-lg flex items-center justify-center text-orange-500 shadow-sm hover:scale-110 transition-all active:scale-95">
                ...
                {getFilesForActiveFolder().map((f: any, i: number) => (
                  <div key={i} className="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-50 group hover:border-indigo-100 transition-all">
                    ...
                    <div className="ml-auto flex items-center gap-2">
                      <button onClick={() => handleViewFile(f)} className="px-3 py-1 bg-white border border-slate-100 text-[10px] rounded-lg text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all">Visualizar</button>
                    </div>
                  </div>
                ))}
